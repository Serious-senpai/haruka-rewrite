...
def _display_frame(task: asyncio.Task) -> str:
    ...
    return f"{frame.f_code.co_name} in {frame.f_code.co_filename}::{frame.f_code.co_firstlineno}"
...
async def tasks_cmd(ctx: commands.Context):
    with open("./tasks.txt", "w", encoding="utf-8") as f:
        f.write("\n".join(f"Task name='{task.get_name()}' done={task.done()} frame={_display_frame(task)}" for task in asyncio.all_tasks()))
