...
class CustomHelpCommand(commands.MinimalHelpCommand):
    ...
    async def send_bot_help(self, mapping: Mapping[Optional[commands.Cog], List[commands.Command]]) -> None:
        ...
        embed.add_field(
            name="💬 General",
            value="```\nabout, avatar, emoji, help, info, ping, prefix, remind, rerun, say, source, speak, svinfo\n```",
            inline=False,
        )
        embed.add_field(
            name="✨ Fun",
        ...
        embed.add_field(
            name="🎶 Music",
            value="```\nadd, download, export, import, pause, play, playlist, queue, register, remove, repeat, resume, rotate, shuffle, skip, stop, stopafter, unregister, vping\n```",
            inline=False,
        )
        embed.add_field(
            name="🛡️ Moderation",
